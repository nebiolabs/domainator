...
Prototype Flask-based web UI for running Domainator tools and workflows.

## Project status

- HTTP API for uploads, tool discovery, workflow catalog, job execution, and log retrieval.
- Initial web UI for file management, tool browsing, and job monitoring.
- Subprocess-backed executor that consumes jsonargparse-compatible config files.
- Schema registry that loads bundled sample tools/workflows.
...
Use `--schema-dir` to point at additional tool or workflow schemas.
