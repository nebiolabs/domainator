...
Domainator uses the GenBank file format as a carrier of both sequence and annotation data. Independence from a fixed set of sequence sources and the co-location of sequences and all their annotation data in a single file increases data portability and decreases complexity for end-users. Domainator can add functional annotations to sequences by comparison to protein HMM-profiles, protein sequences, nucleotide sequences, nucleotide HMMs, or infernal covariance models. Protein references are searched against CDSs or protein records, while nucleic-acid references are searched against whole nucleotide contigs. For example, in a single call to domainate, a set of genome or metagenome contigs can be annotated with hits to Pfam HMM-profiles, RFAM profiles, and hits to REBASE Gold Standard protein sequences at the same time.
...
__Editors__ are programs whose output format is the same as their input format. Each individual editor performs a simple task, but they can be combined in arbitrarily long chains to accomplish complex transformations. Examples are domain_search, which outputs a subset of the input sequences, based on the presence of a hit to a reference sequence or profile; domainate, which outputs all the input sequence but adds domain annotations based on hits to reference sequences; deduplicate_genbank, which performs similarity clustering using CD-HIT, usearch, or DIAMOND and outputs only the cluster representatives input sequences; and select_by_cds, which extracts genome neighborhoods around domains of interest. 
...
|  enum_report.py | from a genbank file generates a tab separated file of properties and metadata related to the input, for example the domain content of each contig or cds. | genbank files | a tab separated, html, or JSON (`--json`, streaming NDJSON) metadata table  | yes | yes |  |
...
|  hmmer_build.py | runs hmmbuild. Unlike the hmmbuild from the HMMER3 package, hmmer_build.py allows specifying accession and description from the command line | an MSA | a .hmm file (optionally gzipped) | no  | no  | Builds amino acid, DNA, and RNA profiles. The alphabet is inferred from the alignment, or set explicitly with `--alphabet`. `--window_length` and `--window_beta` set the nhmmer search window (the profile's MAXL field) for nucleotide profiles. |
|  hmmer_compare.py |  runs local profile alignment, calculating pairwise hmm scores and position-wise alignments |  hmmer3 profiles  |  scores and (optionally) alignments |  yes |  no  | hmmer_compare, hmmer_search, and seq_dist can all be used to compare profiles, with different outputs. Amino acid, DNA, and RNA profiles are all supported, but both sides must use the same alphabet. Scores are on the input alphabet's own scale: nucleotide scores run at roughly half the amino acid scale. |
|  hmmer_report.py |  Writes a tabular report from .hmm files, with a line for each profile | one or more .hmm files | a tab separated, html, or JSON (`--json`, streaming NDJSON) metadata table | yes  | yes  | `--alphabet` reports whether each profile is amino, DNA, or RNA. |
|  hmmer_search.py |  Selects hmm profiles based on similarity scores to a reference set of hmm profiles |  hmmer3 profiles (optionally gzipped)  |  hmmer3 profiles (optionally gzipped) |  yes |  yes  | hmmer_compare, hmmer_search, and seq_dist can all be used to compare profiles, with different outputs. Amino acid, DNA, and RNA profiles are all supported, but `-i` and `-r` must use the same alphabet. The default `--score_cutoff` is scaled to the alphabet, because nucleotide scores run at roughly half the amino acid scale. |
|  hmmer_select.py | Extracts a subset of profiles from .hmm files  | one or more .hmm files (optionally gzipped) | an hmm file (optionally gzipped) | yes  | yes  | All `--input` files must use one alphabet, because HMMER locks an .hmm file to the alphabet of its first profile. `--alphabet` splits a mixed set of files into per-alphabet outputs. |
...
|  build_ssn_viewer.py | builds a `.dsnv` bundle and/or a standalone, dependency-free HTML Domainator Similarity Network Viewer for exploring a sequence similarity network in a browser | a symmetric similarity matrix. optionally, tables of node metadata | a `.dsnv` bundle (gzip JSON) and/or a self-contained HTML file (`--html`, with `--embed_data` to inline the bundle) | no | no | The viewer reconstructs clusters at any threshold from an MST hierarchy, so the bundle scales with node count rather than edge count. Metadata can be edited and annotated in the browser, and "Save session" writes a bundle carrying those edits plus the viewer state. Each metadata column header carries a chart glyph that builds a frequency table, bar or pie chart, histogram, box plot, cumulative distribution or summary statistics over the rows the table is showing, inheriting the column's colors and exportable as SVG, PNG or TSV. See [docs/ssn_viewer.md](docs/ssn_viewer.md) for a guide to the viewer itself. |
|  ssn_navigator.py | read-only navigation of a build_ssn_viewer `.dsnv` bundle: lists clusters, cluster contents, and per-cluster metadata distributions at a chosen similarity threshold | a `.dsnv` Domainator Similarity Network Viewer bundle | compact JSON (overview / thresholds / clusters / cluster / node queries) | no | no | Agent-friendly companion to the interactive HTML Domainator Similarity Network Viewer. |
...
|  structure_dist.py | calculates similarity or distance matrices between protein structures |  structure files (pdb/cif, optionally gzipped; files, a glob, or a directory searched recursively) or a prebuilt aligner database, optionally a second set as the reference  |  a sparse or dense matrix of similarity or distance values  |  no  |  no  | The structure-input counterpart of `seq_dist.py`. Rows are inputs and columns are references, the opposite of the other structure tools, so `--evalue` is computed against the reference set as it is in `seq_dist.py`. Row and column labels are the same record ids `structure_to_genbank.py` writes, so the matrix feeds `build_ssn.py`, `build_tree.py` and `build_ssn_viewer.py` directly. `--mode` adds `tmscore`, `lddt` and `fident` (and their `_dist` forms) to seq_dist's score-based modes; `tmscore` and `lddt` need `--algorithm foldseek`. |
|  structure_domainate.py | annotates protein structures with hits to reference structures | structure files (pdb/cif, optionally gzipped; files, a glob, or a directory searched recursively) or a prebuilt aligner database, plus reference structures or an aligner database | a protein genbank file, one record per structure chain, with `Domainator` annotations | no | yes | The structure-input counterpart of `domainate.py`. Every input chain is written, whether or not it had hits. Because the databases are built from real structures they carry C-alpha coordinates, so `--alignment_type 1` (TM-align) and the `--metrics` tmscore/lddt/rmsd are available with the default foldseek backend. `--algorithm reseek` selects reseek instead. Consider `structure_search.py` when searching a large database with a few references. |
|  structure_search.py | searches a structure database with a small number of reference structures | a prebuilt aligner database (for example a foldseek database built from AlphaFold DB), or structure files to build one from, plus reference structures | a protein genbank file of the hit records, each with a `Domain_Search` annotation | no | yes | The structure-input counterpart of `domain_search.py`. With the default foldseek backend, hit records are built from the sequences the aligner reports, so the searched database is never read end to end. `--algorithm reseek` selects reseek instead. `--max_seqs` bounds how many hits the aligner keeps per reference; raise it for large databases, and note the warning when a reference saturates it. |
|  structure_to_genbank.py | converts protein structures into a protein genbank file | structure files (pdb/cif, optionally gzipped) or a prebuilt aligner database | a protein genbank file, one record per structure chain | no | yes | Makes structures ordinary Domainator inputs, so the sequence-based tools work on them unchanged. `--keep_db` also persists the aligner database, which carries C-alpha coordinates and can be reused as the input or reference of the other two structure tools. `--algorithm` selects foldseek (default) or reseek. |
