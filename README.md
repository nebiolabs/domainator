...
|  domainator_format_db.py | Prepares a `domain_search` database: split into shards (`--shards N`), BGZF-compress (`--compress`), and/or write `.didx` offset indexes. | genbank or fasta file(s), optionally gzip/BGZF | shard files (`<base>.N.<ext>`), optional `.bgz`, and `.didx` index sidecars | no | no | Indexes let `domain_search` skip the per-run offset scan; a logical db name auto-resolves to its shards. |
